...
public class WeedFSClient {
    ...
    public RequestResult write(String path,String mimiType) {
...
//            assignedInfo = gson.fromJson(response.toString(), WeedAssignedInfo.class);
            assignedInfo = JSON.parseObject(response.toString(),new TypeReference<WeedAssignedInfo>(){});;
        ...
        if(mimiType==null||"".equals(mimiType)){
        	mimiType = "text/plain";
        }
        FileBody fileBody = new FileBody(inputFile, mimiType);
...
//            int size = gson.fromJson(response, ResponseEntity.class).getSize();
            int size = JSON.parseObject(response.toString(),new TypeReference<ResponseEntity>(){}).getSize();
...
//            locations = gson.fromJson(response.toString(), ServerLocations.class);
              locations = JSON.parseObject(response.toString(),new TypeReference<ServerLocations>(){});
...
//            locations = gson.fromJson(response.toString(), ServerLocations.class);
    ...
    public static void main(String[] args) {
        ...
        WeedFSClient client = new WeedFSClient("172.19.1.121", "9333");
        try {
            result = client.write("D:\\blog_left.jpg","image/JPEG");
        }
        catch (Exception e) {
